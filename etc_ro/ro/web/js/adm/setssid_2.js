...
define([ 'jquery', 'knockout', 'config/config', 'service', 'underscore' ], function ($, ko, config, service, _) {
        ...
        function WifiBasicVM() {
            ...
            self.wifi_enable = ko.observable(info.tz_ssid3_enable);
            if(info.tz_ssid3_enable==""||info.tz_ssid3_enable==null){
            ...
            self.isShowSSIDInfoDiv33 = ko.observable(false);
            if(config.WIFI_SWITCH_SUPPORT) {
                if(info.tz_ssid3_enable == "1") {
                    self.isShowSSIDInfoDiv33(true);
                } else {
                    self.isShowSSIDInfoDiv33(false);
                }
            } else {
            ...
            self.selectedMode = ko.observable(info.AuthMode3);
            self.passPhrase = ko.observable(info.passPhrase3);
            self.showPassword = ko.observable(false);
            self.ssid = ko.observable(info.SSID3);
            self.broadcast = ko.observable(info.broadcast3 == '1' ? '1' : '0');
            self.apIsolation = ko.observable(info.apIsolation == '1' ? '1' : '0');
            self.cipher = info.cipher3;
            self.selectedStation = ko.observable(info.MAX_Access_num3);
            var MAX_Station_num;
            ...
            self.encryptType = ko.observable(info.encryptType3);
            self.keyID = ko.observable(info.S3_keyID);
            ...
            self.getWepPassword = function(){
                return self.keyID() == '3' ? info.S3_Key4Str1 : (self.keyID() == '2' ? info.S3_Key3Str1 : self.keyID() == '1' ? info.S3_Key2Str1 : info.S3_Key1Str1);
            }
            ...
            self.clear = function (option) {
                if (option == "switch") {
                    ...
                    self.wifi_enable(info.tz_ssid3_enable);
                } else if (option == "ssid1") {
                    self.selectedMode(info.AuthMode3);
                    self.passPhrase(info.passPhrase3);
                    self.ssid(info.SSID3);
                    self.broadcast(info.broadcast3 == '1' ? '1' : '0');
                    self.cipher = info.cipher3;
                    self.selectedStation(info.MAX_Access_num3);
                    self.apIsolation(info.apIsolation == '1' ? '1' : '0');
                    if(config.WIFI_WEP_SUPPORT){
                        self.encryptType(info.encryptType3);
                        self.keyID(info.S3_keyID);
                        self.wepPassword(self.getWepPassword());
                    }
            ...
            self.checkSettings = function (ssid) {
                ...
                if(ssid == "ssid1" || ssid == "ssid2"){                 
                    if(ssid == "ssid1"){
                         var accessDevice = service.getAllOnceDatas().tz_ssid3_station_num;
                ...
                if (config.HAS_MULTI_SSID && info.multi_ssid_enable == "1") {
                    if ((ssid == "ssid1" && parseInt(self.selectedStation()) + parseInt(info.m_MAX_Access_num) > info.MAX_Station_num)
                        || (ssid == "ssid2" && parseInt(self.m_selectedStation()) + parseInt(info.MAX_Access_num3) > info.MAX_Station_num)) {
            ...
            self.saveSSID1Action = function () {
                ...
                if(config.WIFI_WEP_SUPPORT){
                    ...
                    params.wep_key_1 = info.S3_Key1Str1;   
                    params.wep_key_2 = info.S3_Key2Str1;   
                    params.wep_key_3 = info.S3_Key3Str1;
                    params.wep_key_4 = info.S3_Key4Str1;
                    var WEPSelect = '0';
                ...
                service.setWifiBasic3(params, function (result) {
                        ...
                        if (result.result == "success") {
                            if(isWifi){
                                ...
                                addInterval(function(){
                                    var info = getWifiBasic();
                                    service.refreshAPStationStatus();
                                    if(info.tz_ssid3_enable == self.wifi_enable()){
                                        successOverlay();
                                        self.clear();
                                    }
                                }, 1000);
