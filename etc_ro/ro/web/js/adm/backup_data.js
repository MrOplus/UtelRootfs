	...
	function ($, fileinput, service, ko, config, status) {
		...
		fileUploadSubmitClickHandler = function() {
		    ...
		    if(typeof fileName == "undefined" || fileName == '' || fileName == $.i18n.prop("no_file_selected")){
				...
				if (!fileExt.match(/.zip/i) && !fileExt.match(/.gz/i) && !fileExt.match(/.bin/i)) {
			...
			$("#fileUploadForm").attr("action", "/cgi-bin/export_update/" + fileName);
		...
		self.backupData = function() {
            service.backupData({}, function (data) {
                if (data && data.tz_save_user_info == "success") {
                    $('#downloadURL').show();
