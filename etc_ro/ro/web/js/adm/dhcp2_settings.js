...
define([ 'jquery', 'knockout', 'config/config', 'service'],
    ...
    var originfrmLan="";
        function LanVM() {
            ...
            var info = getLanInfo();
            self.ipAddress = ko.observable(info.va1_ipaddr);
            self.ipAddress2 = ko.observable(info.ipAddress);
            self.ipAddress3 = ko.observable(info.va0_ipaddr);
            self.subnetMask = ko.observable(info.va1_netmask);
            self.macAddress = ko.observable(info.macAddress);
            self.dhcpServer = ko.observable(info.tz_dhcp3_enable);
            self.dhcpStart = ko.observable(info.va1_dhcpStart);
            self.dhcpEnd = ko.observable(info.va1_dhcpEnd);
            self.dhcpDns = ko.observable(info.va1_dhcpDns);
            self.dhcpLease = ko.observable(info.va1_dhcpLease);
            ...
            self.save = function(){
             ...
             $("#txtIpAddress").on("blur",function(){
               ...
               if(ipAddress == ipAddress3){
                showAlert("dhcp_dhcp2");
                return false;
            ...
            self.saveAct = function() {
                ...
                service.setLanInfo2(params, function(result) {
